...
type AtreeDatabase = Record<string, ATreeAbility[]>;

type GraphScratchNode = ATreeNode & {
  visited?: boolean;
  assigned?: boolean;
  scc?: unknown;
};
...
export let ATREES: AtreeDatabase | undefined;

/*
 * Load atree info remote DB (aka a big json file).
 */
export async function load_atree_data(version_str: string): Promise<void> {
...
const elem_mastery_abil: ATreeAbility = {
  ...
  blockers: [],
  cost: 0,
  display: { row: 0, col: 0, icon: 'node_0' },
  properties: {},
  effects: [],
};

function defaultMeleeAbility(
  display_name: string,
  desc: string,
  properties: Record<string, number>,
  effect: ATreeAbility['effects'][number],
): ATreeAbility {
  return {
    display_name,
    id: 999,
...
// TODO: Range numbers
export const default_abils: Record<PlayerClass, ATreeAbility[]> = {
    Mage: [
        defaultMeleeAbility('Mage Melee', 'Mage basic attack.', { range: 12 }, default_spells.wand[0] as ATreeAbility['effects'][number]),
        elem_mastery_abil,
    ],
    Warrior: [
        defaultMeleeAbility('Warrior Melee', 'Warrior basic attack.', { range: 4 }, default_spells.spear[0] as ATreeAbility['effects'][number]),
        elem_mastery_abil,
    ],
    Archer: [
        defaultMeleeAbility('Archer Melee', 'Archer basic attack.', { range: 9 }, default_spells.bow[0] as ATreeAbility['effects'][number]),
        elem_mastery_abil,
    ],
    Assassin: [
        defaultMeleeAbility('Assassin Melee', 'Assassin basic attack.', { range: 3 }, default_spells.dagger[0] as ATreeAbility['effects'][number]),
        elem_mastery_abil,
    ],
    Shaman: [
        defaultMeleeAbility('Shaman Melee', 'Shaman basic attack.', { range: 32.25, speed: 0 }, default_spells.relik[0] as ATreeAbility['effects'][number]),
        elem_mastery_abil,
    ],
};
...
export function get_sorted_class_atree(
  ...
  const atree_map = new Map<number, ATreeNode>();
  let atree_head!: ATreeNode;
  for (const i of atree_raw) {
    atree_map.set(i.id, { children: [], parents: [], ability: i });
  ...
  const atree_topo_sort: ATree = [];
  for (const scc of sccs) {
    for (const node of scc.nodes as GraphScratchNode[]) {
      delete node.visited;
 ...
 */
export function abil_can_activate(
  atree_node: ATreeNode,
  atree_state: RenderedATree,
  reachable: Set<number>,
  archetype_count: Map<string, number>,
  points_remain: number,
): ATreeActivationResult {
 ...
 */
export function atree_translate(
  atree_merged: MergedATree,
  v: number | string,
): number {
    if (typeof v === 'string') {
 ...
 */
export class AbilityTreeEnsureNodesNode extends ComputeNode {
    build_node: ComputeNode;
    stat_agg_node: ComputeNode;
    passthrough: PassThroughNode;
    spelldmg_nodes: SpellDamageCalcNode[];
    spell_display_elem: HTMLElement;

    /**
     * Kinda "hyper-node": Constructor takes nodes that should be linked to (build node and stat agg node)
     */
    constructor(build_node: ComputeNode, stat_agg_node: ComputeNode) {
    ...
    compute_func(input_map) {
        ...
        const spell_map = input_map.get('spells') as Map<number, SpellDefinition>;

        for (const [spell_id, spell] of new Map([...spell_map.entries()].sort((a, b) => a[0] - b[0])).entries()) {
...
function generateTooltip(container, node_elem, ability, atree_map) {
    ...
    // archetype req
    if ((ability.archetype_req ?? 0) > 0) {
        let arch_req = make_elem("p", ["scaled-font", "my-0", "mx-1"], {});
        let req_archetype: string;
        if ('req_archetype' in ability && ability.req_archetype !== "") {
            req_archetype = ability.req_archetype;
        }
        else {
...
export function atree_set_state(node_wrapper: RenderedATNode, new_state: boolean): void {
...
const atree_connector_atlas_img = make_elem("img", [], {src: "../media/atree/connectors.png", loaded: false}) as LoadedAtlasImage;
...
const atree_node_atlas_img = make_elem("img", [], {src: "../media/atree/icons.png", loaded: false}) as LoadedAtlasImage;
