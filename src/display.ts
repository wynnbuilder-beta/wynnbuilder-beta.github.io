...
function isSkpId(id: string): boolean {
  return (skp_order as readonly string[]).includes(id);
}

function replaceAllStr(str: string, search: string, replacement: string): string {
  return str.split(search).join(replacement);
}

function apply_elemental_format(p_elem: HTMLElement, id: string, suffix = ''): void {
...
export const itemBGPositions: Record<string, string> = {
...
export function displaySetBonuses(parent_id: string, build: BuildDisplayContext): void {
    ...
    for (const [setName, count] of build.activeSetCounts) {
        ...
        let mock_item = new Map<string, unknown>([
        ["fixID", true],
        ["displayName", setName + " Set: " + count + "/" + active_set.bonuses.length]]) as ExpandedItem;
        let mock_minRolls = new Map<string, unknown>();
        let mock_maxRolls = new Map<string, unknown>();
...
export function displayExpandedItem(_item: ExpandedItem | Map<string, unknown>, parent_id: string): void {
    // Commands to "script" the creation of nice formatting.
    // #commands create a new element.
    // !elemental is some janky hack for elemental damage.
    // normals just display a thing.
    const item: DisplayMap = new Map(_item) as DisplayMap;   // shallow copy
    ...
    let p_elem: HTMLElement | undefined;
    let setName: string;
                ...
                if (id === "slots") {
                    ...
                    for (let major_id_str of item.get(id)) {
                        ...
                        if (major_id_str.includes(":")) {
                            ...
                            b_elem.innerHTML = replaceAllStr(
                                replaceAllStr(
                                    replaceAllStr(
                                        replaceAllStr(
                                            replaceAllStr(
                                                replaceAllStr(b_elem.innerHTML, "[neutral]", "<span class='Neutral'></span>"),
                                                "[earth]", "<span class='Earth'></span>"),
                                            "[thunder]", "<span class='Thunder'></span>"),
                                        "[water]", "<span class='Water'></span>"),
                                    "[fire]", "<span class='Fire'></span>"),
                                "[air]", "<span class='Air'></span>");
                    ...
                    (item_name_elem as HTMLAnchorElement).href = item_link;
                    ...
                    */
                    ...
                    if (!(item.get("tier") === "Crafted" && item.get("category") === "armor" && id === "hp") && (!isSkpId(id)) || (isSkpId(id) && item.get("tier") !== "Crafted" && parent_div.nodeName === "table")) { //skp warp
                        p_elem = displayFixedID(parent_div, id, item.get(id), elemental_format) ?? undefined;
                    } else if (item.get("tier") === "Crafted" && item.get("category") === "armor" && id === "hp") {
                        p_elem = displayFixedID(parent_div, id, item.get(id + "Low") + "-" + item.get(id), elemental_format) ?? undefined;
                    }
                    if (id === "lore" && p_elem) {
                        p_elem.style.cssText = "font-style: italic";
                    } else if (isSkpId(id)) { //id = str, dex, int, def, or agi
                        if (item.get("tier") !== "Crafted") {
...
function displayFixedID(
    active: HTMLElement,
    id: string,
    value: unknown,
    elemental_format: boolean,
    style?: string | null,
    ...
    if (style) {
        ...
        value_elem.textContent = String(value) + idSuffixes[id];
        row.appendChild(value_elem);
        ...
        if (elemental_format) {
            apply_elemental_format(p_elem, id, String(value));
        }
        else {
            p_elem.textContent = idPrefixes[id].concat(String(value), idSuffixes[id]);
        }
...
function displayDefenseStats(parent_elem: HTMLElement, statMap: Map<string, unknown>, insertSummary?: boolean): void {
    ...
    boost.textContent = String(stats[0]);
    ...
    boost = document.createElement("div");
    boost.textContent = String(stats[2]);
    boost.classList.add("col");
    boost.classList.add("text-end");
    ...
    for (let i = 0; i < eledefs.length; i++) {
        ...
        boost.classList.add(Number(eledefs[i]) >= 0 ? "positive" : "negative");
        boost.classList.add("col");
        boost.classList.add("text-end");
...
export function displayPowderSpecials(
    ...
    if (powderSpecials.length === 0) {
        parent_elem.style.cssText = "display: none";
        return;
    }
    parent_elem.style.cssText = "";
    ...
    for (const special of specials) {
        ...
        for (const [key, value] of powder_special.weaponSpecialEffects) {
            if (key === "Damage") {
                ...
                let critChance = skillPointsToPercentage(skillpoints[1] as number);
...
export function getSpellCost(
    ...
    const cost = getBaseSpellCost(stats, spell) * (1 + statNum(stats, 'spPct' + (spell as SpellDefinition).base_spell + 'Final') / 100);
...
function getBaseSpellCost(stats: Map<string, unknown>, spell: SpellDefinition | number): number {
    ...
    return cost * (1 + statNum(stats, "spPct" + s.base_spell) / 100);
}
 ...
 */
export function initItemHoverPopups(eq_keys) {
    ...
    function populatePopup(eq) {
        ...
        const collapsed = tooltip.firstElementChild && (tooltip.firstElementChild as HTMLElement).style.display === 'none';
        popup.innerHTML = '';
    ...
    document.addEventListener('click', (e) => {
        ...
        if (img_loc && img_loc.contains(target)) return;
        dismiss();
    });
    ...
    window.addEventListener('scroll', (e) => {
        const target = e.target as Node | null;
        if (!target) return;
        if (popup.contains(target) || popup === target) return;
        if (state === HOVERING) dismiss();
