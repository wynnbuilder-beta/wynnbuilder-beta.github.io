...
function collapse_element(elmnt: string): void {
  const elem_list = document.querySelector(elmnt)!.children;
  if (elem_list) {
    for (const elem of elem_list) {
      if (elem.classList.contains('no-collapse')) {
        continue;
      }
  ...
  (document.querySelector(elmnt) as HTMLElement).style.removeProperty('display');
}
